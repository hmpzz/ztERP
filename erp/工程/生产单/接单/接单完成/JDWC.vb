...
"                           where a.id=-1"
    ...
    Private Sub ExDataGridView1_CellMouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles ExDataGridView1.CellMouseDown
        If Me.ExDataGridView1.SelectedCells.Count = 0 Then
            Me.接单完成ToolStripMenuItem.Enabled = False
            Me.完成回退ToolStripMenuItem.Enabled = False
        ...
        If Me.ExDataGridView1.Rows(e.RowIndex).Cells("state1").Value = "审核" Then
            Me.接单完成ToolStripMenuItem.Enabled = True
            Me.完成回退ToolStripMenuItem.Enabled = True
        ...
        ElseIf Me.ExDataGridView1.Rows(e.RowIndex).Cells("state2").Value = "已完成" Then
            Me.接单完成ToolStripMenuItem.Enabled = False
            Me.完成回退ToolStripMenuItem.Enabled = True
        ElseIf Me.ExDataGridView1.Rows(e.RowIndex).Cells("state2").Value = "确认完成" Then
            Me.接单完成ToolStripMenuItem.Enabled = False
            Me.完成回退ToolStripMenuItem.Enabled = False
        End If
    End Sub

    Private Sub 接单完成ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 接单完成ToolStripMenuItem.Click
        Dim sqlcon As New SqlConnection
        Dim sqlcmd As New SqlCommand
        Dim sqlad As New SqlDataAdapter
        ...
        If Me.ExDataGridView1.SelectedCells.Count = 0 Then
            MsgBox("请选择要完成的单据！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If
        ...
        sql = "SELECT state,jd_state,money " & _
        ...
        If dt.Rows(0)("state") <> 1 Then
            MsgBox("该单据不是审核状态，不可进行接单操作，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If

        If dt.Rows(0)("money") = 0 Then
            MsgBox("该单据完工金额为0，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If
        ...
        If dt.Rows(0)("jd_state") = 0 Then
            MsgBox("该单据未接单，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        ElseIf dt.Rows(0)("jd_state") = 2 Then
            MsgBox("该单据已完成，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        ElseIf dt.Rows(0)("jd_state") = 3 Then
            MsgBox("该单据已确认完成，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If

        sql = "update  scd_master set jd_state=2,jd_date=getdate(),jd_man='" & rs_name & "' where billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "'"
        sqlcmd.CommandText = sql
        sqlcmd.ExecuteNonQuery()

        'Refresh_ExDataGridView(Me.TreeView1.SelectedNode.Text)
        Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("state2").Value = "已完成"
    ...
    Private Sub 完成回退ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 完成回退ToolStripMenuItem.Click
        Dim sqlcon As New SqlConnection
        Dim sqlcmd As New SqlCommand
        Dim sqlad As New SqlDataAdapter
        ...
        If Me.ExDataGridView1.SelectedCells.Count = 0 Then
            MsgBox("请选择要接单的单据！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If
        ...
        If dt.Rows(0)("jd_state") = 0 Then
            MsgBox("该单据没有接单，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        ElseIf dt.Rows(0)("jd_state") = 1 Then
            MsgBox("该单据为接单状态，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        ElseIf dt.Rows(0)("jd_state") = 3 Then
            MsgBox("该单据已确认完成，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        ...
        sql = "update  scd_master set jd_state=1,jd_date=getdate(),jd_man='" & rs_name & "' where billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "'"
        sqlcmd.CommandText = sql
        sqlcmd.ExecuteNonQuery()

        'Refresh_ExDataGridView(Me.TreeView1.SelectedNode.Text)
        Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("state2").Value = "已接单"

        Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("check_man").Value = rs_name
        Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("check_date").Value = Now

        MsgBox("回退成功！", MsgBoxStyle.OkOnly, "提示")
    End Sub
End Class