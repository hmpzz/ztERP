...
"                           where a.id=-1"
    ...
    Private Sub ExDataGridView1_CellMouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles ExDataGridView1.CellMouseDown
        If Me.ExDataGridView1.SelectedCells.Count = 0 Then
            Me.接单确认ToolStripMenuItem.Enabled = False
            Me.接单回退ToolStripMenuItem.Enabled = False
        ...
        If Me.ExDataGridView1.Rows(e.RowIndex).Cells("state1").Value = "审核" Then
            Me.接单确认ToolStripMenuItem.Enabled = True
            Me.接单回退ToolStripMenuItem.Enabled = True
    ...
    Private Sub 接单确认ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 接单确认ToolStripMenuItem.Click
        Dim sqlcon As New SqlConnection
        Dim sqlcmd As New SqlCommand
        Dim sqlad As New SqlDataAdapter
        ...
        MsgBox("接单成功！", MsgBoxStyle.OkOnly, "提示")
    End Sub

    Private Sub 接单回退ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 接单回退ToolStripMenuItem.Click
        Dim sqlcon As New SqlConnection
        Dim sqlcmd As New SqlCommand
        Dim sqlad As New SqlDataAdapter
        ...
        ElseIf dt.Rows(0)("jd_state") = 3 Then
            MsgBox("该单据已确认完成，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        ElseIf dt.Rows(0)("money") <> 0 Then
            MsgBox("该单据已确认价格，无法接单后退，请检查后操作！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If

        sql = "update  scd_master set jd_state=0,jd_date=getdate(),jd_man='" & rs_name & "' where billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "'"
        sqlcmd.CommandText = sql
        sqlcmd.ExecuteNonQuery()

        'Refresh_ExDataGridView(Me.TreeView1.SelectedNode.Text)
        Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("state2").Value = "未接单"
