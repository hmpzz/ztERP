...
Public Class t_part_in
    ...
    "                where t_rs.bm_id= " & BM_id & s1 & s & " order by a.billno"
        ...
        Me.ExDataGridView1.AddColumn("ll_man", "退库人")
...
"                           where a.id=-1"
    ...
    Private Sub ToolStripButton_add_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton_add.Click
        Dim t_part_in_add As New t_part_in_add
        t_part_in_add.Owner = Me
        t_part_in_add.state = "new"
        t_part_in_add.billno = ""

        t_part_in_add.ShowDialog()
        If t_part_in_add.DialogResult = Windows.Forms.DialogResult.OK Then
            Refresh_ExDataGridView_master()
        End If
    End Sub
    ...
    Private Sub ToolStripButton_mod_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton_mod.Click
        ...
        sql = "select  count(*) from T_part_in_master where state=0 and billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "'"
        sqlcmd.CommandText = sql
        ...
        sql = "select  count(*) from T_part_in_detail where ok_amount<>0 and billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "'"
        sqlcmd.CommandText = sql

        If sqlcmd.ExecuteScalar > 0 Then
            MsgBox("该单据已经有出库信息，不可以修改！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If
        ...
        Dim t_part_in_add As New t_part_in_add
        t_part_in_add.Owner = Me
        t_part_in_add.state = "mod"
        t_part_in_add.billno = Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value
    ...
    Private Sub ToolStripButton_del_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton_del.Click
        ...
        If MsgBox("确认要删除退库单号为：" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "的单据吗？", MsgBoxStyle.YesNo, "提示") = MsgBoxResult.No Then
            Exit Sub
        End If
        ...
        Try
            ...
            sql = "SELECT count(*)  " & _
                           "FROM T_part_in_master  where state<>0 and billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "' "
            ...
            sql = "delete from T_part_in_master where billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "' "
            sqlcmd.CommandText = sql
            sqlcmd.ExecuteNonQuery()

            sql = "delete from T_part_in_detail where billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "' "
            sqlcmd.CommandText = sql
            sqlcmd.ExecuteNonQuery()
