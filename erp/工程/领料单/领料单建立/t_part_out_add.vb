...
Public Class t_part_out_add
    ...
    Private Sub t_part_out_add_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
...
"                             where a.billno='" & billno & "' order by a.id"
            ...
            Me.TextBox1.Text = CType(Me.Owner, t_part_out).ExDataGridView1.Rows(CType(Me.Owner, t_part_out).ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value
            Me.TextBox2.Text = CType(Me.Owner, t_part_out).ExDataGridView1.Rows(CType(Me.Owner, t_part_out).ExDataGridView1.SelectedCells(0).RowIndex).Cells("project_no").Value
            Me.ComboBox1.Text = CType(Me.Owner, t_part_out).ExDataGridView1.Rows(CType(Me.Owner, t_part_out).ExDataGridView1.SelectedCells(0).RowIndex).Cells("depot_name").Value
            Me.TextBox4.Text = CType(Me.Owner, t_part_out).ExDataGridView1.Rows(CType(Me.Owner, t_part_out).ExDataGridView1.SelectedCells(0).RowIndex).Cells("ll_man").Value
            Me.TextBox3.Text = CType(Me.Owner, t_part_out).ExDataGridView1.Rows(CType(Me.Owner, t_part_out).ExDataGridView1.SelectedCells(0).RowIndex).Cells("memo").Value
    ...
    Private Sub 添加NEWToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 添加NEWToolStripMenuItem.Click
        Dim t_part_out_add_detail As New t_part_out_add_detail
        Dim i As Int32
        Dim s As String = ""
        ...
        For i = 0 To Me.ExDataGridView1.Rows.Count - 1
            s = s & "'" & Me.ExDataGridView1.Rows(i).Cells("item_code").Value & "',"
        Next
        ...
        t_part_out_add_detail.Owner = Me
        t_part_out_add_detail.id = s
        t_part_out_add_detail.depot_code = Me.ComboBox2.Text
        t_part_out_add_detail.ShowDialog()
    ...
    Private Sub ToolStripButton_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton_save.Click
        ...
        Try
            If state = "new" Then
                ...
                sql = "update t_dj_num set num=num+1 where  tt='LL-" & Me.ComboBox2.Text.Trim & "'"
                sqlcmd.CommandText = sql
                sqlcmd.ExecuteNonQuery()

                sql = "select * from t_dj_num where tt='LL-" & Me.ComboBox2.Text.Trim & "'"
                sqlcmd.CommandText = sql
                sqlad.SelectCommand = sqlcmd
                dt.Rows.Clear()
                sqlad.Fill(dt)
                ...
                sql = "INSERT INTO [T_part_out_master] " & _
                        "           ([billno] " & _
                        "           ,[project_no] " & _
                        "           ,[input_man] " & _
                        "           ,[input_date] " & _
                ...
                For i = 0 To Me.ExDataGridView1.Rows.Count - 1
                    sql = "INSERT INTO [T_part_out_detail] " & _
                        "           ([billno] " & _
                        "           ,[item_code] " & _
                        "           ,[amount] " & _
                        "           ,[OK_amount]) " & _
            ...
            ElseIf state = "mod" Then
                ...
                '如果已经不是新建就不能修改
                sql = "select count(*) from T_part_out_master where state=0 and billno='" & billno & "'"
                sqlcmd.CommandText = sql
                If sqlcmd.ExecuteScalar = 0 Then
                    Throw New System.Exception("该单据不是新建状态，不可以修改！")
                End If

                sql = "UPDATE [T_part_out_master] " & _
                ...
                For i = 0 To Me.ExDataGridView1.Rows.Count - 1
                    If Me.ExDataGridView1.Rows(i).Cells("id").Value <> 0 Then
                        ...
                        sql = "UPDATE [T_part_out_detail] " & _
                ...
                sql = "delete from T_part_out_detail where  billno='" & billno & "' " & s
                sqlcmd.CommandText = sql
                sqlcmd.ExecuteNonQuery()
        ...
        If state = "new" Then
            CType(Me.Owner, t_part_out).Refresh_ExDataGridView_master()
        ElseIf state = "mod" Then
