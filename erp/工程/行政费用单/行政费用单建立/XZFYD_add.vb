...
Public Class XZFYD_add
    ...
    Private Sub ToolStripButton_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton_save.Click
        ...
        If Me.TextBox2.Text.Trim.Length = 0 Then
            MsgBox("请选择部门！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If
        ...
        Try
            If state = "new" Then
                ...
                sql = "update t_dj_num set num=num+1 where  tt='XZFY'"
                sqlcmd.CommandText = sql
                sqlcmd.ExecuteNonQuery()

                sql = "select * from t_dj_num where tt='XZFY'"
                sqlcmd.CommandText = sql
                ...
                sql = "INSERT INTO [t_xzfy_master] " & _
                        "           ([billno] " & _
                        "           ,[bmno] " & _
                        "           ,[input_man] " & _
                        "           ,[input_date] " & _
                ...
                For i = 0 To Me.ExDataGridView1.Rows.Count - 1
                    sql = "INSERT INTO [t_xzfy_detail] " & _
                        "           ([billno] " & _
                        "           ,[memo] " & _
                        "           ,[money] ) " & _
                        "     VALUES " & _
            ...
            ElseIf state = "mod" Then
                ...
                '如果已经不是新建就不能修改
                sql = "select count(*) from t_xzfy_master where state=0 and billno='" & billno & "'"
                sqlcmd.CommandText = sql
                If sqlcmd.ExecuteScalar = 0 Then
                    Throw New System.Exception("该单据不是新建状态，不可以修改！")
                End If

                sql = "UPDATE [t_xzfy_master] " & _
                    "   SET [bmno] ='" & Me.TextBox5.Text & "' " & _
                ...
                sql = "delete from t_xzfy_detail where  billno='" & billno & "' " & s
                sqlcmd.CommandText = sql
                sqlcmd.ExecuteNonQuery()
        ...
        If state = "new" Then
            CType(Me.Owner, XZFYD).Refresh_ExDataGridView_master()
        ElseIf state = "mod" Then
    ...
    Private Sub XZFYD_add_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Dim sqlcon As New SqlConnection
        Dim sqlcmd As New SqlCommand
        Dim sqlad As New SqlDataAdapter
...
"                             where a.billno='" & billno & "' order by a.id"
            ...
            Me.TextBox1.Text = CType(Me.Owner, XZFYD).ExDataGridView1.Rows(CType(Me.Owner, XZFYD).ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value
            Me.TextBox5.Text = CType(Me.Owner, XZFYD).ExDataGridView1.Rows(CType(Me.Owner, XZFYD).ExDataGridView1.SelectedCells(0).RowIndex).Cells("bmno").Value
            Me.TextBox2.Text = CType(Me.Owner, XZFYD).ExDataGridView1.Rows(CType(Me.Owner, XZFYD).ExDataGridView1.SelectedCells(0).RowIndex).Cells("bmmc").Value
            Me.TextBox4.Text = CType(Me.Owner, XZFYD).ExDataGridView1.Rows(CType(Me.Owner, XZFYD).ExDataGridView1.SelectedCells(0).RowIndex).Cells("sq_man").Value
            Me.TextBox3.Text = CType(Me.Owner, XZFYD).ExDataGridView1.Rows(CType(Me.Owner, XZFYD).ExDataGridView1.SelectedCells(0).RowIndex).Cells("memo").Value
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        Dim XZFY_add_BM As New XZFY_add_BM
        XZFY_add_BM.Owner = Me
        Me.Tag = ""
        If XZFY_add_BM.ShowDialog() = Windows.Forms.DialogResult.OK Then
            Me.TextBox5.Text = Me.Tag
            Me.Tag = ""

            sqlcon = getconn()
            sqlcmd.Connection = sqlcon
            sql = "select bmmc from t_bm where bmno='" & Me.TextBox5.Text & "'"
    ...
    Private Sub 添加NEWToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 添加NEWToolStripMenuItem.Click
        Dim XZFYD_add_detail As New XZFYD_add_detail

        XZFYD_add_detail.Owner = Me
        XZFYD_add_detail.ShowDialog()
