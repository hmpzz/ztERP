...
Public Class t_user_depot
    ...
    Private Sub cellclick(ByVal index As Int32)
        ...
        sql = "select a.*,isnull(b.id,0) as check1 " & _
                "from  t_depot a left join   " & _
                "	(select * from t_user_depot where login_id='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells.Item(0).RowIndex).Cells("login_id").Value & "' ) b on a.depot_code=b.depot_code where a.stop=1 "
        ...
        Me.ExDataGridView2.AddColumn("depot_code", "仓库代码")
        Me.ExDataGridView2.AddColumn("depot_name", "仓库名称")
    ...
    Private Sub ToolStripButton_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton_save.Click
        ...
        Try
            ...
            sql = "delete from t_USER_DEPOT where login_id='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells.Item(0).RowIndex).Cells("login_id").Value & "'"
            sqlcmd.CommandText = sql
            ...
            For i = 0 To Me.ExDataGridView2.Rows.Count - 1
                If Not IsDBNull(Me.ExDataGridView2.Rows(i).Cells("check1").Value) Then
                    If Me.ExDataGridView2.Rows(i).Cells("check1").Value <> 0 Then
                        sql = "insert into t_user_depot(login_id,depot_code) values ('" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells.Item(0).RowIndex).Cells("login_id").Value & "','" & Me.ExDataGridView2.Rows(i).Cells("depot_code").Value & "')"
                        sqlcmd.CommandText = sql
                        sqlcmd.ExecuteNonQuery()
                    End If
                End If
