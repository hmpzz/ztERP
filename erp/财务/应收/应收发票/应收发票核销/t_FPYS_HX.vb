...
Public Class t_FPYS_HX
    ...
    Private Sub Refresh_ExDataGridView_master(Optional ByVal s As String = "")
        ...
        sql = "select a.*,case state when 0 then '新建' when 1 then '审核' when 2 then '结案' end as state1, case hx when 0 then '未核销' when 1 then '已核销' end as hx1,case kp_state when 0 then '未开票' when 1 then '已开票' end as kp_state1  " & _
                ...
                " where 1=1 " & s & " and a.state=1 and hx=0  order by a.billno "
    ...
    Private Sub ToolStripButton_save_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ToolStripButton_save.Click
        ...
        sql = "select count(*) from t_fpys_master where kp_state=1 and state=1 and billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "' "

        sqlcmd.CommandText = sql
        If sqlcmd.ExecuteScalar() = 0 Then
            MsgBox("该记录状态不是审核或者没有填写发票号，请刷新后再试！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If
        ...
        sql = "update T_FPYS_master set hx=1,hx_man='" & rs_name & "',hx_date=getdate() where billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "' "
        sqlcmd.CommandText = sql
    ...
    Private Sub 修改ToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles 修改ToolStripMenuItem.Click
        Dim t_FPYS_HX_mod As New t_FPYS_HX_mod
        ...
        sql = "select count(*) from  t_fpys_master where hx=0 and billno='" & Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value & "' "
        sqlcmd.Connection = sqlcon
        sqlcmd.CommandText = sql
        If sqlcmd.ExecuteScalar() = 0 Then
            MsgBox("该记录已经被删除或者是核销，请刷新后再试！", MsgBoxStyle.OkOnly, "提示")
            Exit Sub
        End If
        ...
        t_FPYS_HX_mod.Owner = Me
        t_FPYS_HX_mod.billno = Me.ExDataGridView1.Rows(Me.ExDataGridView1.SelectedCells(0).RowIndex).Cells("billno").Value

        t_FPYS_HX_mod.ShowDialog()

    End Sub

    Private Sub t_FPYS_HX_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
